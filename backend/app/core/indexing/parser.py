...
def parse_docx(file_bytes: bytes, filename: str) -> list[dict]:
    # FIX: python-docx is installed as 'python-docx' but imported as 'docx'
...
def parse_image(file_bytes: bytes, filename: str, caption_fn=None) -> list[dict]:
    """
    If a caption_fn is supplied (callable: bytes → str), use it.
    ...
    if caption_fn:
        ...
        caption = (
            ...
            "No captioning model is configured. "
            "Describe the image content manually or integrate a vision model.]"
        )
...
def parse_document(
    file_bytes: bytes,
    filename: str,
    caption_fn: Optional[object] = None,
) -> list[dict]:
    """
